...
@include('landing.components.topHeaderMenu')
  ...
  <main class="achievement-container">
    ...
    <header class="achievement-header">
      ...
      <article id="achievementContent" class="achievement-content" style="display:none"></article>
    ...
    <!-- Cover Image -->
    <figure id="coverSection" class="achievement-cover" style="display:none">
      <img id="coverImage" alt="Cover image" loading="lazy"/>
    </figure>

    <!-- Attachments -->
    <section class="achievement-attachments" id="attachmentsSection" style="display:none">
...
@include('landing.components.footer')
  ...
  <script>
    (function() {
      ...
      // Find achievement object in response
      function findAchievementObject(payload) {
        if (!payload) return null;

        if (payload.data && typeof payload.data === 'object') return payload.data;
        if (payload.achievement && typeof payload.achievement === 'object') return payload.achievement;
        if (payload.item && typeof payload.item === 'object') return payload.item;
        if (payload.data && payload.data.data && typeof payload.data.data === 'object') return payload.data.data;
        if (typeof payload === 'object' && (payload.title || payload.body || payload.uuid)) return payload;
      ...
      async function load() {
        ...
        if (!uuid) {
          setLoading(false);
          setError('No UUID found in the URL.');
          return;
        }
        ...
        const candidates = [
          `/api/public/achievements/${encodeURIComponent(uuid)}`,
          `/public/achievements/${encodeURIComponent(uuid)}`,
          `/api/achievements/${encodeURIComponent(uuid)}`
        ];
        ...
        setLoading(false);
        setError('Achievement not found or API endpoint is not reachable. Please verify your public show route URL (expected: /api/public/achievements/{uuid}).');
      }
