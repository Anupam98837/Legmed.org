{{-- resources/views/modules/dashboard/itPersonDashboard.blade.php --}}
@section('title','IT Person Dashboard')
...
<style>
/* =========================
 * IT Person Dashboard (MSIT theme)
 * - Fully dynamic from GET /api/it-person/dashboard (default)
...
.it-wrap{max-width:1200px;margin:18px auto 48px;padding:0 12px;overflow:visible}
...
.it-hero-inner{position:relative;z-index:1;display:flex;gap:14px;align-items:flex-start;justify-content:space-between;flex-wrap:wrap}
.it-hero-left{min-width:260px;flex:1}
.it-hero-title{font-size:26px;font-weight:800;letter-spacing:-.2px;margin:0;font-family:var(--font-head)}
.it-hero-sub{margin:8px 0 0;font-size:14px;opacity:.92}
.it-hero-meta{display:flex;gap:10px;flex-wrap:wrap;margin-top:12px}
.it-chip{
...
.it-chip i{opacity:.95}

/* Grid */
.it-grid{margin-top:14px;display:grid;grid-template-columns:repeat(12, minmax(0,1fr));gap:14px;align-items:stretch}
.it-grid > div{display:flex}
.it-grid > div > .it-card{width:100%;height:100%}
...
.it-card-sub{font-size:12.5px;color:var(--muted-color);margin-top:3px}
.it-card-body{padding:14px 16px}
.it-card-foot{
  padding:12px 16px;
...
.it-kv .k{font-size:12px;color:var(--muted-color)}
.it-kv .v{font-weight:800;color:var(--ink);word-break:break-word}
@media (min-width: 768px){
  .it-kv{grid-template-columns:repeat(2, minmax(0,1fr))}
}
...
@media (max-width: 992px){
  .it-col-8,.it-col-6,.it-col-4{grid-column:span 12}
...
@section('content')
<div class="it-wrap" id="itDashWrap" data-endpoint="{{ $endpoint ?? '/api/it-person/dashboard' }}">
  ...
  <div class="it-hero">
    <div class="it-hero-inner">
      <div class="it-hero-left">
        <h1 class="it-hero-title" id="heroTitle">Welcome 👋</h1>
        <div class="it-hero-sub" id="heroSub">Loading your dashboard…</div>

        <div class="it-hero-meta">
          <span class="it-chip"><i class="fa-solid fa-user-shield"></i> <span id="chipRole">—</span></span>
          <span class="it-chip"><i class="fa-solid fa-building-user"></i> <span id="chipDept">—</span></span>
          <span class="it-chip"><i class="fa-regular fa-clock"></i> <span id="chipUpdated">—</span></span>
        </div>
      </div>
  ...
  <div class="it-grid">
    ...
    <div class="it-col-12" id="basicDetailsCard">
      <div class="it-card">
        <div class="it-card-head">
          <div>
            <div class="it-card-title"><i class="fa-solid fa-id-card"></i> Basic Details</div>
            <div class="it-card-sub" id="basicSub">Your profile summary</div>
          </div>
        </div>
        <div class="it-card-body">
          <div id="basicBox" class="it-kv">
          ...
          <div class="it-empty mt-3 d-none" id="deptMissingNote">
            <i class="fa-regular fa-circle-info me-1"></i>
            Department is not selected. Update your profile to see department-wise dashboard.
          </div>
        </div>
        <div class="it-card-foot">
    ...
    <div class="it-col-12 d-none" id="deptKpiCard">
      <div class="it-card">
        <div class="it-card-head">
          <div>
            <div class="it-card-title"><i class="fa-solid fa-gauge-high"></i> Department Overview</div>
            <div class="it-card-sub">Department KPIs (dynamic)</div>
          </div>
        </div>
        <div class="it-card-body">
    ...
    {{-- Quick Actions --}}
    <div class="it-col-6" id="quickCard">
      <div class="it-card">
        <div class="it-card-head">
          <div>
            <div class="it-card-title"><i class="fa-solid fa-bolt"></i> Quick Actions</div>
            <div class="it-card-sub">Shortcuts for your IT tasks</div>
          </div>
        </div>
        <div class="it-card-body" id="quickActions">
          <div class="skel w80"></div>
    ...
    {{-- Alerts --}}
    <div class="it-col-6" id="alertsCard">
      <div class="it-card">
        <div class="it-card-head">
          <div>
            <div class="it-card-title"><i class="fa-solid fa-circle-exclamation"></i> Alerts</div>
            <div class="it-card-sub">Needs your attention</div>
          </div>
        </div>
        <div class="it-card-body" id="alertsBox">
          <div class="skel w80"></div>
          <div class="skel w60" style="margin-top:10px"></div>
          <div class="skel w40" style="margin-top:10px"></div>
        </div>
    ...
    <div class="it-col-12 d-none" id="deptActivityCard">
      <div class="it-card">
        <div class="it-card-head">
          <div>
            <div class="it-card-title"><i class="fa-solid fa-wave-square"></i> Department Activity</div>
            <div class="it-card-sub" id="activitySub">Last 7 days</div>
          </div>
        </div>
        <div class="it-card-body">
    ...
    <div class="it-col-12 d-none" id="deptRecentCard">
      <div class="it-card">
        <div class="it-card-head">
          <div>
            <div class="it-card-title"><i class="fa-solid fa-list"></i> Recent Department Updates</div>
            <div class="it-card-sub" id="recentSub">Latest updates</div>
          </div>
        </div>
        <div class="it-card-body">
...
<script>
(function(){
  if (window.__IT_DASH_INIT__) return;
  window.__IT_DASH_INIT__ = true;

  const token = sessionStorage.getItem('token') || localStorage.getItem('token') || '';
  if (!token) { window.location.href = '/'; return; }

  const wrap = document.getElementById('itDashWrap');
  const preferredEndpoint = wrap?.dataset?.endpoint || '/api/it-person/dashboard';

  // Fallbacks (only tried if preferred fails)
  const endpointCandidates = [
    preferredEndpoint,
    '/api/it-person/dashboard'
  ...
  function renderBasicDetails(data, hero, deptObj, hasDept){
    ...
    const role  = safeStr(hero.role || data.role || 'it_person');
    ...
    basicHint.textContent = safeStr(
      ...
      hasDept ? 'You can manage IT operations based on your access level.' : 'Select department to see department-wise data.'
    );
  ...
  function renderQuickActions(actions, hasDept){
    if(!isArr(actions) || !actions.length){
      const defaults = hasDept ? [
        { title:'Open Support Queue', url:'#', icon:'fa-headset', hint:'View pending IT requests' },
        { title:'System Checklist', url:'#', icon:'fa-list-check', hint:'Daily checks & tasks' },
        { title:'Update Profile', url:'/it-person/profile', icon:'fa-user-pen', hint:'Manage your account details' },
      ] : [
        { title:'Update Profile', url:'/it-person/profile', icon:'fa-user-pen', hint:'Select your department to unlock dashboard' },
      ];
      actions = defaults;
    }
  ...
  function renderAlerts(alerts, hasDept){
    if(!isArr(alerts) || !alerts.length){
      if (!hasDept) {
        ...
        alertsBox.innerHTML = `<div class="it-empty">No alerts 🎉</div>`;
        return;
      }
    }
  ...
  async function loadDashboard(){
    ...
    try{
      ...
      chipRole.textContent    = safeStr(hero.role, (data.role || 'it_person'));
      ...
      basicBox.innerHTML = `<div class="it-empty">No data (API error).</div>`;
      quickActions.innerHTML = `<div class="it-empty">No data (API error).</div>`;
      alertsBox.innerHTML = `<div class="it-empty">No data (API error).</div>`;
