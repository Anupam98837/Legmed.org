{{-- resources/views/modules/author/authorDashboard.blade.php --}}
  ...
  <div class="ad-hero">
    <div class="ad-hero-inner">
      <div class="ad-hero-left">
        <h1 class="ad-hero-title" id="heroTitle">Author Dashboard</h1>
        <div class="ad-hero-sub" id="heroSub">Loading dashboard data…</div>

        <div class="ad-hero-meta">
          <span class="ad-chip"><i class="fa-solid fa-user-pen"></i> <span id="chipRole">—</span></span>
...
<script>
(function(){
  if (window.__AUTHOR_DASH_INIT__) return;
  window.__AUTHOR_DASH_INIT__ = true;

  const PAGE_TITLE = 'Author Dashboard';

  // Force top/header title if layout has any of these common nodes
  function setTopHeaderTitle(t){
    const text = (t || PAGE_TITLE).toString();
    document.title = text;
  ...
  setTopHeaderTitle(PAGE_TITLE);

  const token = sessionStorage.getItem('token') || localStorage.getItem('token') || '';
  if (!token) { window.location.href = '/'; return; }

  const ROLE_LABEL = (sessionStorage.getItem('role') || localStorage.getItem('role') || 'AUTHOR').toString().toUpperCase();
  ...
  async function loadDashboard(){
    ...
    try{
      // ✅ keep using admin API (same as director)
      ...
      // ✅ FIX: never use API hero.title (it returns "Admin Dashboard")
      const hero = isObj(data.hero) ? data.hero : {};
      heroTitle.textContent = PAGE_TITLE;                 // FORCE
      heroSub.textContent = safeStr(hero.sub, '—');       // ok to use
      chipRole.textContent = ROLE_LABEL || 'AUTHOR';      // FORCE role label
      chipUpdated.textContent = fmtDateTime(hero.updated_at || data.updated_at || new Date().toISOString());

      // also force layout header (page indicator) to Author Dashboard
      setTopHeaderTitle(PAGE_TITLE);
      ...
      heroTitle.textContent = PAGE_TITLE;
      setTopHeaderTitle(PAGE_TITLE);
