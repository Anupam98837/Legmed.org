{{-- resources/views/modules/admin/adminDashboard.blade.php --}}
...
<style>
/* =========================
 * Admin Dashboard (MSIT theme)
 * - Fully dynamic from GET /api/admin/dashboard
 * - No static numbers
  ...
  <div class="ad-hero">
    <div class="ad-hero-inner">
      <div class="ad-hero-left">
        <h1 class="ad-hero-title" id="heroTitle">Admin Dashboard</h1>
      ...
      <div class="ad-card">
        <div class="ad-card-head">
          ...
          <div class="small text-muted">
            Loads from <code>GET /api/admin/dashboard</code>
          </div>
        </div>
...
<script>
(function(){
  if (window.__ADMIN_DASH_INIT__) return;
  window.__ADMIN_DASH_INIT__ = true;
  ...
  function normalizeDashboardPayload(raw){
    /**
     * We support multiple backend shapes so you don't have to rewrite front-end
     * Expected best:
     * { success:true, data:{ hero:{title,sub,role,updated_at}, kpis:[], activity:{labels,values,sub,hint}, quick_actions:[], alerts:[], recent:{columns, rows, hint, sub} } }
     *
     * Minimal:
     * { success:true, data:{ ... } } or { ... } (if controller returns plain JSON)
     */
  ...
  function renderRecent(recent){
    // recent can be: { columns:[{key,label,align}], rows:[{...}], hint, sub }
  ...
  function renderActivity(activity){
    // activity can be { labels:[], values:[], sub, hint } OR { points:[{label,value}] }
    ...
    if (!L.length || !V.length) {
      // show empty look by creating an "empty" chart
  ...
  async function loadDashboard(){
    ...
    try{
      // ✅ Your endpoint (protected by checkRole:admin,director,principal)
      ...
      heroTitle.textContent = safeStr(hero.title, 'Admin Dashboard');
      heroSub.textContent = safeStr(hero.sub, '—');
      chipRole.textContent = safeStr(hero.role, (data.role || 'ADMIN'));
      ...
      // hard empty states (no fake values)
