...
.ce-carousel-slide img{
  width:100%;
  height:100%;
  object-fit:cover;
  display:block;
}
.ce-carousel[data-fit="contain"] .ce-carousel-slide img{
  object-fit:contain;
  background:#fff;
}
...
.ce-carousel-btn:hover{ background:rgba(17,24,39,.75); }
...
.ce-carousel-dot.active{ background:#fff; }

/* Editor canvas: don't let carousel controls steal clicks/drags */
#ceCanvasEdit .ce-carousel-btn,
#ceCanvasEdit .ce-carousel-dots{ pointer-events:none; }
  ...
  <div class="ce-editor">
    ...
    <aside class="ce-components-panel">
      ...
      <div class="ce-components-list ce-active" id="list-elements">
        ...
        <div class="ce-component" draggable="true" data-key="ce-carousel"
  data-html="<div class='ce-carousel'
    data-images='https://placehold.co/600x260|https://placehold.co/600x260?text=2|https://placehold.co/600x260?text=3'
    data-autoplay='false'
    data-interval='3000'
    data-arrows='true'
    data-dots='true'
    ...
    <div class='ce-carousel-viewport' style='height:260px;'>
      <div class='ce-carousel-track'>
        <div class='ce-carousel-slide'><img src='https://placehold.co/600x260' alt='Slide'></div>
        <div class='ce-carousel-slide'><img src='https://placehold.co/600x260?text=2' alt='Slide'></div>
        <div class='ce-carousel-slide'><img src='https://placehold.co/600x260?text=3' alt='Slide'></div>
      </div>
    </div>
    <button type='button' class='ce-carousel-btn ce-carousel-prev' aria-label='Previous'>&lsaquo;</button>
    <button type='button' class='ce-carousel-btn ce-carousel-next' aria-label='Next'>&rsaquo;</button>
    <div class='ce-carousel-dots'></div>
  </div>">
  <i class="fa-regular fa-images"></i> Carousel
</div>
...
block.querySelectorAll('a').forEach((el,i)=>{
  ...
  addButtonContentField(contentPane, el, i);
    ...
    addCarouselEditors(contentPane, stylePane, block);
...
addPDFEditors(contentPane, stylePane, block);
  ...
  function addImageStyleEditors(panel, block){
    ...
    imgs.forEach((img, idx)=>{
      if(img.closest('.ce-carousel')) return; // ✅ carousel has its own editor
...
/* =========================
   CAROUSEL (runtime + inspector)
========================= */
function ceParseUrlList(raw){
  return (raw || '')
    .split(/\r?\n|\|/g)
    .map(s => s.trim())
    .filter(Boolean);
...
function ceCarouselGetOpts(car){
  const h = parseInt(car.dataset.height || '260', 10);
  const interval = parseInt(car.dataset.interval || '3000', 10);

  return {
    height: Math.max(120, Number.isNaN(h) ? 260 : h),
    interval: Math.max(800, Number.isNaN(interval) ? 3000 : interval),
    autoplay: car.dataset.autoplay === 'true',
    arrows: car.dataset.arrows !== 'false',
    dots: car.dataset.dots !== 'false',
    loop: car.dataset.loop !== 'false',
    fit: (car.dataset.fit === 'contain') ? 'contain' : 'cover'
  };
}

function ceEnsureCarouselMarkup(car){
  let viewport = car.querySelector('.ce-carousel-viewport');
  let track = car.querySelector('.ce-carousel-track');

  if(!viewport){
    viewport = document.createElement('div');
    viewport.className = 'ce-carousel-viewport';
    car.prepend(viewport);
  }
  if(!track){
    track = document.createElement('div');
    track.className = 'ce-carousel-track';
    viewport.appendChild(track);
  } else if(track.parentElement !== viewport){
    viewport.appendChild(track);
  }

  let prev = car.querySelector('.ce-carousel-prev');
  let next = car.querySelector('.ce-carousel-next');

  if(!prev){
    prev = document.createElement('button');
    prev.type = 'button';
    prev.className = 'ce-carousel-btn ce-carousel-prev';
  ...
  if(!next){
    ...
    next.type = 'button';
    next.className = 'ce-carousel-btn ce-carousel-next';
    next.setAttribute('aria-label','Next');
    next.innerHTML = '&rsaquo;';
    car.appendChild(next);
  }

  let dots = car.querySelector('.ce-carousel-dots');
  if(!dots){
    dots = document.createElement('div');
    dots.className = 'ce-carousel-dots';
    car.appendChild(dots);
  }

  return { viewport, track, prev, next, dots };
}

function ceCarouselGetImages(car){
  let urls = ceParseUrlList(car.dataset.images || '');
  if(!urls.length){
    urls = Array.from(car.querySelectorAll('.ce-carousel-slide img'))
      .map(img => (img.getAttribute('src') || '').trim())
      .filter(Boolean);
  }
  if(!urls.length) urls = ['https://placehold.co/600x260'];
  car.dataset.images = urls.join('|'); // ✅ export-safe
  return urls;
}

function ceCarouselStopAutoplay(car){
  if(car.__ceCarTimer){
    clearInterval(car.__ceCarTimer);
    car.__ceCarTimer = null;
  }
}

function ceCarouselStartAutoplay(car){
  const opts = ceCarouselGetOpts(car);
  ceCarouselStopAutoplay(car);
  if(!opts.autoplay) return;

  car.__ceCarTimer = setInterval(()=>{
    const urls = ceCarouselGetImages(car);
    const max = urls.length - 1;
    let idx = parseInt(car.dataset.index || '0', 10);
    if(Number.isNaN(idx)) idx = 0;
    ceCarouselGo(car, idx + 1, false);
  }, opts.interval);
}

function ceCarouselGo(car, idx, restartAuto){
  const opts = ceCarouselGetOpts(car);
  const { track, dots, prev, next } = ceEnsureCarouselMarkup(car);
  ...
  dots.querySelectorAll('.ce-carousel-dot').forEach((b, di)=>{
    b.classList.toggle('active', di === i);
  });
  ...
  if(!opts.loop){
    ...
    prev.style.opacity = '1'; next.style.opacity = '1';
  }

  if(restartAuto) ceCarouselStartAutoplay(car);
}

function ceCarouselRender(car){
  const opts = ceCarouselGetOpts(car);
  const { viewport, track, dots, prev, next } = ceEnsureCarouselMarkup(car);

  car.dataset.fit = opts.fit;
  viewport.style.height = opts.height + 'px';

  const urls = ceCarouselGetImages(car);

  track.innerHTML = urls.map(u =>
    '<div class="ce-carousel-slide"><img src="' + ceEscapeAttr(u) + '" alt="Slide"></div>'
  ).join('');

  dots.innerHTML = urls.map((_,i)=>
    '<button type="button" class="ce-carousel-dot" data-idx="' + i + '" aria-label="Go to slide ' + (i+1) + '"></button>'
  ).join('');

  prev.style.display = opts.arrows ? '' : 'none';
  next.style.display = opts.arrows ? '' : 'none';
  dots.style.display = opts.dots ? 'flex' : 'none';

  let idx = parseInt(car.dataset.index || '0', 10);
  if(Number.isNaN(idx) || idx > urls.length-1) idx = 0;
  car.dataset.index = String(idx);

  ceCarouselGo(car, idx, false);
  ceCarouselStartAutoplay(car);

  // bind once (delegation)
  if(!car.__ceCarBound){
    car.__ceCarBound = true;

    car.addEventListener('click', (e)=>{
      const p = e.target.closest('.ce-carousel-prev');
      const n = e.target.closest('.ce-carousel-next');
      const d = e.target.closest('.ce-carousel-dot');

      if(p){
        e.preventDefault();
        const cur = parseInt(car.dataset.index || '0', 10) || 0;
        ceCarouselGo(car, cur - 1, true);
        return;
      }
      if(n){
        e.preventDefault();
      ...
      if(d){
        e.preventDefault();
        const to = parseInt(d.dataset.idx || '0', 10) || 0;
        ceCarouselGo(car, to, true);
      }
    });

    // pause/resume (works in export + code preview)
    car.addEventListener('mouseenter', ()=> ceCarouselStopAutoplay(car));
    car.addEventListener('mouseleave', ()=> ceCarouselStartAutoplay(car));
  }
}

function initCarousels(root){
  (root || document).querySelectorAll('.ce-carousel').forEach(car=>{
    ceCarouselRender(car);
  });
}

/* ===== Inspector: Carousel ===== */
function addCarouselEditors(panel, stylePanel, block){
  const car = block.querySelector('.ce-carousel');
  if(!car || isUI(car)) return;
  ...
  f1.className = 'ce-field';
  f1.innerHTML = `
    <label>Carousel Images (one URL per line)</label>
    <textarea class="ce-car-urls" placeholder="https://...">${urls.join('\n')}</textarea>
    <div class="ce-muted" style="padding:6px 0 0 0;">Paste full https image URLs. Empty lines are ignored.</div>
  `;
  const ta = f1.querySelector('.ce-car-urls');

  let armed=false, tmr=null;
  const applyUrls = ()=>{
    const list = ceParseUrlList(ta.value.replace(/\r/g,''));
    car.dataset.images = (list.length ? list : ['https://placehold.co/600x260']).join('|'); // ✅ export-safe
    car.dataset.index = '0';
    ceCarouselRender(car);
    syncExport();
  };
  ...
  f2.className = 'ce-field';
  f2.innerHTML = `
    <label>Carousel Controls</label>

    <label style="margin-top:8px;">Height (px)</label>
    <input type="number" class="ce-car-h" min="120" value="${opts.height}">

    <label style="margin-top:8px;">Image Fit</label>
    <select class="ce-car-fit">
      <option value="cover" ${opts.fit==='cover'?'selected':''}>Cover</option>
      <option value="contain" ${opts.fit==='contain'?'selected':''}>Contain</option>
    </select>

    <div style="margin-top:10px;display:flex;gap:10px;flex-wrap:wrap;">
      <label style="display:flex;gap:6px;align-items:center;">
        <input type="checkbox" class="ce-car-autoplay" ${opts.autoplay?'checked':''}> Autoplay
      </label>
      <label style="display:flex;gap:6px;align-items:center;">
        <input type="checkbox" class="ce-car-loop" ${opts.loop?'checked':''}> Loop
      </label>
      <label style="display:flex;gap:6px;align-items:center;">
        <input type="checkbox" class="ce-car-arrows" ${opts.arrows?'checked':''}> Arrows
      </label>
      <label style="display:flex;gap:6px;align-items:center;">
        <input type="checkbox" class="ce-car-dots" ${opts.dots?'checked':''}> Dots
      </label>
    </div>

    <label style="margin-top:10px;">Autoplay Interval (ms)</label>
    <input type="number" class="ce-car-interval" min="800" value="${opts.interval}">
  `;

  const hInp = f2.querySelector('.ce-car-h');
  const fitSel = f2.querySelector('.ce-car-fit');
  const ap = f2.querySelector('.ce-car-autoplay');
  const loop = f2.querySelector('.ce-car-loop');
  const arrows = f2.querySelector('.ce-car-arrows');
  const dots = f2.querySelector('.ce-car-dots');
  const intInp = f2.querySelector('.ce-car-interval');

  function apply(){
    car.dataset.height = String(Math.max(120, parseInt(hInp.value||'260',10) || 260));
    car.dataset.fit = fitSel.value;
    car.dataset.autoplay = ap.checked ? 'true' : 'false';
    car.dataset.loop = loop.checked ? 'true' : 'false';
    car.dataset.arrows = arrows.checked ? 'true' : 'false';
    car.dataset.dots = dots.checked ? 'true' : 'false';
    car.dataset.interval = String(Math.max(800, parseInt(intInp.value||'3000',10) || 3000));
    ceCarouselRender(car);
    syncExport();
  }

  [hInp, fitSel, ap, loop, arrows, dots, intInp].forEach(el=>{
    el.addEventListener(el.tagName==='SELECT' ? 'change' : (el.type==='checkbox' ? 'change' : 'input'), ()=>{
      pushHistory();
      apply();
    });
  });
  ...
  const rad = parseInt(cs.borderTopLeftRadius,10) || 6;
  const bw  = parseInt(cs.borderWidth,10) || 1;
  const bc  = rgb2hex(cs.borderColor);
  const bg  = rgb2hex(cs.backgroundColor);

  const s = document.createElement('div');
  s.className = 'ce-field';
  s.innerHTML = `
    <label>Carousel Frame</label>
    <div style="display:flex;gap:8px;align-items:center;">
      <input type="number" class="ce-car-rad" min="0" value="${rad}" style="width:90px;" title="Radius">
      <input type="number" class="ce-car-bw" min="0" value="${bw}" style="width:90px;" title="Border width">
      <input type="color" class="ce-car-bc" value="${bc}" style="width:34px;height:34px;padding:0;border:none;" title="Border color">
      <input type="color" class="ce-car-bg" value="${bg}" style="width:34px;height:34px;padding:0;border:none;" title="Background">
    </div>
  `;

  const radInp = s.querySelector('.ce-car-rad');
  const bwInp  = s.querySelector('.ce-car-bw');
  const bcInp  = s.querySelector('.ce-car-bc');
  const bgInp  = s.querySelector('.ce-car-bg');

  function applyStyle(){
    car.style.borderRadius = (parseInt(radInp.value||'0',10) || 0) + 'px';
    car.style.borderWidth  = (parseInt(bwInp.value||'0',10) || 0) + 'px';
    car.style.borderStyle  = 'solid';
    car.style.borderColor  = bcInp.value;
    car.style.backgroundColor = bgInp.value;
    syncExport();
  }

  [radInp,bwInp,bcInp,bgInp].forEach(inp=>{
